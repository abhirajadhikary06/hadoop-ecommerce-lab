...
grp = GROUP orders_fact BY (customer_id, customer_name);

top_customers = FOREACH grp GENERATE
    group.customer_id,
    group.customer_name,
    COUNT(orders_fact) AS total_orders,
    SUM(orders_fact.item_total) AS total_spent;

STORE top_customers
INTO '/gold/orders/top_customers'
USING PigStorage(',');